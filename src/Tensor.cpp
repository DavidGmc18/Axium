...
//TODO make this function instead of macro?
#define CUER(call)                                                                                 \
do {                                                                                               \
...
Tensor::Tensor(std::initializer_list<size_t> dims_, Dtype dtype_, Device device_, size_t alignment_, bool zero_init)
: TensorDescriptor(dims_, dtype_, alignment_), device(device_) {
    switch (device) {
        case CPU: 
            data = aligned_alloc(32, bytes); 
            if (zero_init) memset(data, 0, bytes);
            break;
        case CUDA: 
            CUER(cudaMalloc((void**)&data, bytes));
            if (zero_init) CUER(cudaMemset(data, 0, bytes));
            break;
        default: std::cerr << "Unknown device"; std::abort();
    }
}

Tensor::~Tensor() {
    switch (device) {
        case CPU: free(data); break;
        case CUDA: CUER(cudaFree(data)); break;
        default: std::cerr << "Unknown device, can't deserialize tensor"; std::abort();
    } 
}
